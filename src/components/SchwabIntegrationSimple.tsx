...
export function SchwabIntegrationSimple() {
  ...
  const [lastPriceDetails, setLastPriceDetails] = useState<
    Array<{
      entityId?: string;
      ticker?: string;
      operation?: string;
  ...
  const [expandedLogId, setExpandedLogId] = useState<string | undefined>(
    undefined
  );
  ...
  const oauthMutation = useMutation({
    mutationFn: async (redirectUri: string) => {
      console.log(
        "🔗 [UI] Starting OAuth flow with redirect URI:",
        redirectUri
      );
  ...
  const syncPricesMutation = useMutation({
    mutationFn: async (symbols?: string[]) => {
      console.log(
        "💰 [UI] Starting Schwab prices sync for symbols:",
        symbols || "all symbols"
      );
    ...
    onSuccess: (data) => {
      ...
      if (Array.isArray(maybeDetails)) {
        setLastPriceDetails(maybeDetails as Array<{
          entityId?: string;
          ticker?: string;
          operation?: string;
          changes?: string | Record<string, unknown>;
  ...
  const syncAllMutation = useMutation({
    mutationFn: async () => {
      ...
      if (heldTickers.length === 0) {
        console.warn(
          "⚠️ [UI] No held positions found after holdings sync; skipping price sync"
        );
      ...
      try {
        console.log(
          "🟡 [UI] Starting Yahoo fundamentals sync for held tickers missing data"
        );
        ...
        console.warn(
          "⚠️ [UI] Yahoo fundamentals sync encountered an issue:",
          yErr
        );
      }
      return pricesResult;
  ...
  useEffect(() => {
    ...
    if (!hasExpandedSelection && anyPending && Array.isArray(syncLogs)) {
      const running = syncLogs.find((l: { status?: string; id?: string }) => l?.status === "RUNNING");
  ...
  const handleConnect = async () => {
    ...
    if (
      ...
      console.log(
        "🔒 [UI] Converted localhost to HTTPS for Schwab requirement"
      );
    }
  ...
  const handlePricesSyncHeld = async () => {
    try {
      ...
      const heldTickers = await getHeldPositionTickersServerFn();
      console.log(
        `📊 [UI] Found ${heldTickers.length} held position tickers:`,
        heldTickers
      );
  ...
  const handlePricesSyncFive = async () => {
    try {
      ...
      console.log(
        `🎯 [UI] Updating prices for ${topFive.length} holdings:`,
        topFive
      );

      if (topFive.length === 0) {
        console.warn(
          "⚠️ [UI] No held positions found, skipping partial price sync"
        );
        return;
      }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Connect your Charles Schwab account to automatically import
            accounts, holdings, and prices.
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex items-center justify-between">
            ...
            {isConnected ? (
              ...
              <Button
                ...
                disabled={isConnecting || oauthMutation.isPending}
              >
                  ...
                  (syncLogs ?? []).map((log, _idx: number) => {
                    ...
                    return (
                      <div key={log.id} className="text-sm">
                        <div
                          className="p-3 grid grid-cols-[auto_1fr_auto_auto_1fr] gap-3 items-center cursor-pointer select-none"
                          onClick={() => {
                            ...
                            <div className="rounded border bg-white">
                              ...
                              <div className="max-h-[220px] overflow-auto">
                                {(() => {
                                  ...
                                  const list = (log.syncType === "SECURITIES"
          ...
          <CardContent className="pt-6">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground max-w-md mx-auto">
                Link your Charles Schwab account to automatically import your
                accounts and holdings. Your credentials are encrypted and stored
                securely.
              </p>
              <div className="pt-2">
