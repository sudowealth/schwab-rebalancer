...
export function WashSaleTooltip({
  washSaleInfo,
  className = "",
}: WashSaleTooltipProps) {
  const tooltipContent = generateWashSaleTooltipContent(washSaleInfo);

  return (
    <SimpleTooltip content={tooltipContent}>
      <AlertTriangle
        className={`h-3 w-3 text-yellow-500 flex-shrink-0 ${className}`}
      />
    </SimpleTooltip>
  );
}
...
function generateWashSaleTooltipContent(
  washSaleInfo?: UIWashSaleInfo | null
): string {
