...
export class RebalanceError extends Error {
  constructor(message: string, public portfolioId?: string, public originalError?: unknown) {
    super(message);
...
export function handleServerError(error: unknown): Response {
  ...
  if (error instanceof ExternalServiceError) {
    logError(error, "External service error", { service: error.service, originalError: error.originalError });
    return new Response(`${error.service} service unavailable`, { status: 503 });
  }

  if (error instanceof RebalanceError) {
    logError(error, "Rebalance error", { portfolioId: error.portfolioId, originalError: error.originalError });
