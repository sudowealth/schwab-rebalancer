...
function calculateInflationAdjustedAmount(
  ...
  return baseAmount * (1 + inflationRate / 100) ** (year - 1);
}
...
export function calculateFinancialPlan(inputs: PlanInputs, goals: Goal[] = []): PlanResult {
  ...
  for (let year = 1; year <= inputs.simulationPeriod; year++) {
    ...
    ); // Include details for modal display
    ...
    const inflationFactor = (1 + inputs.inflationRate / 100) ** (year - 1);
  ...
  const totalContributions = yearlyResults.reduce(
    (sum, year) =>
      sum + year.contributionsTaxable + year.contributionsRoth + year.contributionsDeferred,
    0,
  );
  const totalWithdrawals = yearlyResults.reduce(
    (sum, year) => sum + year.withdrawalsTaxable + year.withdrawalsRoth + year.withdrawalsDeferred,
    0,
  );
