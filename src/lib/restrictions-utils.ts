...
export const checkTransactionHistoryForWashSale = (ticker: string, transactions: Transaction[] = []): { hasRisk: boolean; info: WashSaleInfo | null } => {
  ...
  const recentLossSells = transactions.filter(
    (tx) => {
      const executedDate = new Date(tx.executedAt);
      const isRecentSell = tx.ticker === ticker &&
        tx.type === "SELL" &&
        executedDate >= thirtyOneDaysAgo;
      
      // Only check taxable accounts for wash sale rules
      const isTaxableAccount = tx.accountType === "TAXABLE" || 
        (tx.accountName && tx.accountName.toLowerCase().includes('taxable'));
      ...
      if (tx.ticker === ticker && isRecentSell && isWashSaleCandidate) {
        console.log(`🔍 WASH SALE DETECTED: ${ticker} sold at loss $${Math.abs(tx.realizedGainLoss/100).toFixed(2)} on ${executedDate.toLocaleDateString()} in ${tx.accountName}`);
      }
  ...
  if (recentLossSells.length > 0) {
    ...
    const daysAgo = Math.floor(
      (now.getTime() - sellDate.getTime()) / (24 * 60 * 60 * 1000)
    );
...
export class WashSaleRestrictionChecker implements RestrictionChecker {
  ...
  constructor(washSaleRestrictions: WashSaleRestriction[] = [], transactions: Transaction[] = []) {
    ...
    this.restrictionMap = new Map(
      activeRestrictions.map((r) => [r.ticker, r])
    );
    
    if (this.restrictedTickers.size > 0) {
      console.log(`🚫 WASH SALE RESTRICTED SECURITIES: ${Array.from(this.restrictedTickers).join(', ')}`);
    }
  }
...
export function validateTradeAgainstRestrictions(
  ...
  if (restrictionResult.isRestricted && action === "BUY") {
    console.log(
      `🚫 BLOCKED: Cannot BUY ${ticker} - ${restrictionResult.reason}`
    );
    return {
      isAllowed: false,
...
>(
  ...
  for (const security of sortedSecurities) {
    const restrictionResult = restrictionChecker.isSecurityRestricted(
      security.securityId
    );
