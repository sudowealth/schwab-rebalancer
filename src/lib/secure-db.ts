...
export class SecureDB {
  ...
  async getAccountById(accountId: string) {
    const [account] = await this.db
      ...
      .from(schema.account)
      .where(
        and(eq(schema.account.id, accountId), eq(schema.account.userId, this.authContext.userId)),
      )
      .limit(1);
    if (!account) {
  ...
  async updateAccount(accountId: string, updates: Partial<typeof schema.account.$inferInsert>) {
