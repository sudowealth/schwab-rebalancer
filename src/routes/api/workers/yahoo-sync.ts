...
export const Route = createFileRoute('/api/workers/yahoo-sync')({
  ...
  loader: async (ctx) => {
    ...
    const url = new globalThis.URL(request?.url || 'http://localhost/api/workers/yahoo-sync');
    const providedKey = request?.headers.get('x-cron-key') ?? url.searchParams.get('key') ?? '';
    const expectedKey = (process.env.CRON_KEY as string | undefined) || '';

    if (!expectedKey || providedKey !== expectedKey) {
      throw new globalThis.Response(JSON.stringify({ success: false, error: 'Unauthorized' }), {
        status: 401,
    ...
    try {
      ...
      throw new globalThis.Response(JSON.stringify({ success: false, error: message }), {
        status: 500,
        headers: { 'content-type': 'application/json' },
      });
    }
